...
@section("content")
  <!-- Hero section -->
  <section id="index-hero" class="relative h-96 bg-cover bg-left">
    ...
    <div class="z-2 absolute bottom-24 left-6 flex flex-col items-start gap-6 pr-2 text-white md:left-20">
      ...
      <a class="transform rounded-[8px] bg-blue-600 px-6 py-3 text-lg text-slate-100 duration-200 ease-in-out hover:bg-blue-700"
        href="{{ route("shop") }}">Shop now</a>
    </div>
  </section>

  <!-- Bestselleres section -->
  <section class="mx-4 flex flex-col items-center gap-8 pt-6 md:mt-12 lg:mx-16 xl:mx-24 2xl:mx-auto 2xl:max-w-[1280px]">
    <h2 class="text-center text-3xl font-bold text-slate-800">Bestsellers</h2>
    <ul class="mt-4 grid grid-cols-1 gap-6 gap-y-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      @foreach ($popularProducts as $popularProduct)
        <x-product-card :stock_item=$popularProduct />
      @endforeach
    </ul>
    <a class="transform rounded-[8px] bg-blue-600 px-6 py-3 text-lg text-slate-100 duration-200 ease-in-out hover:bg-blue-700"
      href="{{ route("shop") }}">Shop now</a>
  ...
  <section class="mx-4 flex flex-col items-center gap-8 pt-12 md:mt-8 xl:mx-24 2xl:mx-auto 2xl:max-w-[1280px]">
    ...
    <ul class="mt-4 grid grid-cols-1 gap-6 gap-y-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      @foreach ($bestSellingProducts as $bestSellingProduct)
        <x-product-card :stock_item=$bestSellingProduct />
      @endforeach
    </ul>
    <a class="transform rounded-[8px] bg-blue-600 px-6 py-3 text-lg text-slate-100 duration-200 ease-in-out hover:bg-blue-700"
      href="{{ route("shop") }}">Shop now</a>
  ...
  <section id="about-us"
    ...
    <div class="mt-4 flex flex-col items-center gap-4 md:flex-row md:items-start md:justify-normal md:gap-6">
      <div class="overflow-hidden rounded-[8px] border border-transparent md:w-1/2">
        <img class="h-auto w-full rounded-[8px] border border-transparent object-cover"
      ...
      <div class="font-lg flex w-full flex-col justify-start gap-4 text-slate-700 md:w-1/2 md:gap-8">
        <p class="">
        ...
        <a href="#"
          class="text-center text-lg font-bold text-blue-500 underline hover:text-blue-600 md:text-start">Read full
          story</a>
      </div>
    </div>
  </section>
  ...
  <section class="mx-4 flex flex-col items-center gap-8 pt-12 md:mt-12 lg:mx-16 xl:mx-24 2xl:mx-auto 2xl:max-w-[1280px]">
    <h2 class="text-center text-3xl font-bold capitalize text-slate-800">
      Trusted by you
    </h2>
        ...
        <div class="cm-carousel-item w-full flex-shrink-0">
          <div class="flex flex-col gap-4 md:flex-row lg:gap-8">
            <div class="overflow-hidden rounded-[8px] md:w-1/3">
              <img class="h-full w-full max-w-full rounded-[8px] object-cover" alt="Lewis Hamilton in F1 car"
    ...
    <div id="circle-btns" class="flex items-center justify-center gap-3">
      ...
      <button id="left-mobile-btn" class="h-8 w-8 lg:hidden">
        <svg class="stroke-slate-700 transition duration-200 ease-in-out hover:stroke-blue-500 active:stroke-blue-600"
      ...
      @for ($i = 0; $i < 6; $i++)
        <button
          class="cm-circle-btn h-4 w-4 rounded-full border-2 border-slate-500 transition duration-200 ease-in-out hover:border-slate-700"></button>
      @endfor
...
@push("scripts")
  @vite(["resources/js/carousel-handler.js", "resources/js/add-to-cart-btn-handlers.js"])
@endpush
