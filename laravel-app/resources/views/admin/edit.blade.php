    ...
    <main class="flex max-w-screen-xl flex-col px-2 lg:mb-8 xl:m-auto">
        ...
        >
            <form
                ...
                action="{{ url('admin', [$product_stock->first()->product->id]) }}"
                method="post"
            ...
            >
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input
                        ...
                        required
                        maxlength="50"
                        value="{{ $product_stock->first()->product->name }}"
                    />
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <textarea
                        ...
                        rows="4"
                        required
                    >{{ $product_stock->first()->product->main_description}}</textarea>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <textarea
                        ...
                        maxlength="600"
                        rows="4"
                    >{{ $product_stock->first()->product->secondary_description}}</textarea>
                </div>

                <div class="flex flex-col gap-2">
                    <p class="block font-bold">Media</p>
                    <!-- Existing images display -->
                    <div id="imagePreviewOld" class="mt-4 flex flex-wrap justify-center gap-2">
                        @foreach ($product_stock->first()->product->images as $index => $image)
                            <div class="relative group">
                                <img src="{{ url('/images/' . $image->name) }}" alt="{{ $image->name }}"
                                     class="h-32 w-32 object-cover">
                                @if ($image->is_main)
                                    <span class="absolute top-0 left-0 p-1 bg-blue-500 text-white text-xs">Main</span>
                                @endif
                                <!-- Deletion mark -->
                                <button type="button"
                                        class="absolute top-1 right-1 text-white bg-red-600 p-1 text-xs opacity-0 group-hover:opacity-100"
                                        onclick="markForDeletion(this, {{ $index }})">
                                    X
                                </button>
                                <input type="hidden" name="images_to_delete[]" disabled value="{{ $image->id }}">
                            </div>
                        @endforeach
                    ...
                    <!-- image -->
                    <div class="flex flex-col gap-2">
                        <p class="block font-bold">Add new</p>
                        <label
                            for="uploadFile"
                ...
                PNG, JPG, SVG, WEBP, and GIF are Allowed.
                        ...
                        >
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category }}"
                                        @if ($category == $product_stock->first()->product->category)
                                            selected="selected"
                                    @endif
                                >{{ $category }}</option>
                            @endforeach
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        >
                            ...
                            @foreach($vendors as $vendor)
                                <option value="{{ $vendor }}"
                                        @if ($vendor == $product_stock->first()->product->vendor)
                                            selected="selected"
                                    @endif
                                >{{ $vendor }}</option>
                            @endforeach
                    ...
                    <ul class="flex flex-col gap-4" id="product-entries">
                        ...
                        @foreach($product_stock as $stock_record)
                            <x-flavour-size_admin_edit :stock_record="$stock_record" :iteration="($loop->index)+1"
                                                       :flavours="$flavours" :sizes="$sizes"/>
                        @endforeach
...
@push('scripts')
    <script>
        ...
        let product_stock = @json($product_stock);

        function markForDeletion(element, index) {
            const hiddenInput = element.nextElementSibling; // Assumes the input follows the button
            if (hiddenInput.disabled) {
                hiddenInput.disabled = false; // Enable the input to send its value with the form
                element.parentNode.style.opacity = '0.4';
            } else {
                hiddenInput.disabled = true; // Disable the input to prevent sending its value
