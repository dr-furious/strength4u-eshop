...
@section("content")
  <!-- Hero section-->
  <section id="shop-hero" class="relative h-96 bg-cover bg-left">
    ...
    <div class="opacity-65 absolute inset-0 bg-black"></div>

    <div class="z-2 absolute bottom-24 left-6 flex flex-col items-start gap-6 pr-2 text-white md:left-20">
  ...
  <section class="mx-4 flex flex-col items-center gap-4 pt-12 md:mt-8 lg:gap-12 xl:mx-24 2xl:mx-auto 2xl:max-w-[1280px]">
    ...
    <div
      ...
      <button id="display-filter-btn"
        ...
        <p>Filter</p>
        <svg class="flex items-center fill-current" xmlns="http://www.w3.org/2000/svg" height="24"
          viewBox="0 -960 960 960" width="24">
          <path
            d="M440-160q-17 0-28.5-11.5T400-200v-240L168-736q-15-20-4.5-42t36.5-22h560q26 0 36.5 22t-4.5 42L560-440v240q0 17-11.5 28.5T520-160h-80Zm40-308 198-252H282l198 252Zm0 0Z" />
        </svg>
      </button>
      <button id="display-ordering-btn"
        class="flex gap-2 rounded-[8px] border border-slate-300 bg-slate-200 px-4 py-2 shadow-md hover:bg-slate-300 active:bg-slate-300">
        <p>Order</p>
        <svg class="flex items-center fill-current" xmlns="http://www.w3.org/2000/svg" height="24"
          viewBox="0 -960 960 960" width="24">
    ...
    <div id="ordering-menu"
      ...
      <ul class="flex flex-col items-start gap-8 text-xl text-slate-400 lg:flex-row lg:items-center">
        <li>
          <button
            class="cm-order-btn {{ empty(request("orderBy")) || request("orderBy") == "popular" ? "text-slate-700" : "" }} hover:text-slate-700"
            value="popular">
            Popular
          </button>
        </li>
        <li>
          <button class="cm-order-btn {{ request("orderBy") == "newest" ? "text-slate-700" : "" }} hover:text-slate-700"
            value="newest">Newest</button>
        </li>
        <li>
          <button class="cm-order-btn {{ request("orderBy") == "discount" ? "text-slate-700" : "" }} hover:text-slate-700"
            value="discount">Discount</button>
        </li>
        <li>
          <button class="cm-order-btn {{ request("orderBy") == "priceUp" ? "text-slate-700" : "" }} hover:text-slate-700"
            value="priceUp">Price up</button>
        </li>
        <li>
          <button
            class="cm-order-btn {{ request("orderBy") == "priceDown" ? "text-slate-700" : "" }} hover:text-slate-700"
            value="priceDown">
            Price down
          </button>
        </li>
      </ul>
      <button id="hide-ordering-btn" class="fixed right-0 top-0 p-4 lg:hidden">
    ...
    <div class="flex min-w-full flex-col items-start gap-16 lg:flex-row">
      <!-- Filters -->
      <form id="filter-menu" action="{{ route("shop") }}#shop" method="GET"
        ...
        <button id="hide-filter-btn" class="fixed right-0 top-0 p-4 lg:hidden">
          <svg class="inline-flex items-center fill-slate-800" xmlns="http://www.w3.org/2000/svg" height="32"
            viewBox="0 -960 960 960" width="32">
            <path
              d="M256-192.348 192.348-256l224-224-224-224L256-767.652l224 224 224-224L767.652-704l-224 224 224 224L704-192.348l-224-224-224 224Z" />
        ...
        <div class="flex items-center justify-between py-4 text-2xl font-semibold text-slate-800 lg:pt-0">
          <p>Filter By:</p>
        </div>

        <!-- Price Range -->
        <div class="border-b border-b-slate-400 border-opacity-50 pb-3 text-slate-700">
          <div class="flex items-center justify-between">
            ...
            <button type="button" id="price-btn"
              ...
              <svg class="cm-filter-chevron-icon rotate-90 fill-current transition duration-200 ease-in-out"
                xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                <path d="M504-480 320-664l56-56 240 240-240 240-56-56 184-184Z" />
              </svg>
            </button>
          </div>
          <div id="price-list" class="mt-4 max-h-80 overflow-y-auto font-medium transition-all duration-200 ease-in-out">
            <!-- Price Range Slider -->
            <!-- Display Current Value -->
            <p class="mt-2">
              Current Price: ${{ floor($minPrice) }} - $<span
                id="currentPrice">{{ request("priceRange", ceil($maxPrice)) }}</span>
            </p>
            <input type="range" id="priceRange" min="{{ floor($minPrice) }}" max="{{ ceil($maxPrice) }}"
              value="{{ request("priceRange", ceil($maxPrice)) }}" class="w-full cursor-pointer" name="priceRange" />
        ...
        <div class="relative border-b border-b-slate-400 border-opacity-50 pb-3 text-slate-700">
          <div class="flex items-center justify-between">
          ...
          <div id="category-list"
            class="mt-4 max-h-80 overflow-y-auto font-medium transition-all duration-200 ease-in-out">
        ...
        <div class="relative border-b border-b-slate-400 border-opacity-50 pb-3 text-slate-700">
          <div class="flex items-center justify-between">
            ...
            <button type="button" id="flavour-btn"
              class="rounded-full p-1 transition duration-200 ease-in-out hover:bg-slate-600 hover:bg-opacity-10">
              <svg class="cm-filter-chevron-icon fill-current transition duration-200 ease-in-out"
                xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                <path d="M504-480 320-664l56-56 240 240-240 240-56-56 184-184Z" />
              </svg>
            </button>
          ...
          <!-- Dropdown List -->
          <div id="flavour-list"
            class="mt-4 max-h-0 overflow-y-auto font-medium transition-all duration-200 ease-in-out">
        ...
        <div class="relative border-b border-b-slate-400 border-opacity-50 pb-3 text-slate-700">
          <div class="flex items-center justify-between">
            ...
            <button type="button" id="brand-btn"
              class="rounded-full p-1 transition duration-200 ease-in-out hover:bg-slate-600 hover:bg-opacity-10">
              <svg class="cm-filter-chevron-icon fill-current transition duration-200 ease-in-out"
                xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                <path d="M504-480 320-664l56-56 240 240-240 240-56-56 184-184Z" />
          ...
          <!-- Dropdown List -->
          <div id="brand-list" class="mt-4 max-h-0 overflow-y-auto font-medium transition-all duration-200 ease-in-out">
        ...
        <button type="submit"
          class="bg-primary-gray-100 mt-4 cursor-pointer rounded-[8px] border border-slate-500 px-4 py-2 text-slate-700 transition-colors duration-200 hover:bg-slate-100">
          Apply Filters
        </button>
        <a href="{{ route("shop") }}#shops"
          class="font-regular mt-4 cursor-pointer px-4 py-2 text-center text-slate-700 underline transition-colors duration-200 hover:scale-105">
          Clear All Filters
        </a>
      </form>

      <!-- Products -->
      <ul class="grid h-auto max-h-fit grid-cols-1 gap-6 gap-y-10 sm:grid-cols-2 md:grid-cols-3 lg:w-3/4">
        @forelse ($data as $item)
          <x-product-card :stock_item=$item />
        @empty
          <p class="col-span-3 h-fit rounded-[8px] bg-white p-4 text-center text-slate-500">No products match the
            criteria</p>
        @endforelse
      </ul>
    </div>
    ...
    <!-- Pagination -->
    <nav class="ml-auto w-full pb-8 pt-4 lg:w-3/4 lg:pl-16 lg:pt-8">
      {{ $data->appends(request()->except("page"))->links("vendor.pagination.custom-pagination") }}
    </nav>
  </section>
...
@push("scripts")
  @vite(["resources/js/ordering-filter_handlers.js", "resources/js/add-to-cart-btn-handlers.js"])
@endpush
